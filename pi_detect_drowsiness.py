...
def nose_aspect_ratio(nosevalue):

	nar = euclidean_dist(nosevalue[0], nosevalue[6])
...
while True:
	...
	for (x, y, w, h) in rects:
		...
		nosevalue = shape[nStart:nEnd]
		leftEAR = eye_aspect_ratio(leftEye)
		rightEAR = eye_aspect_ratio(rightEye)
		nar = nose_aspect_ratio(nosevalue)
		...
		noseHull = cv2.convexHull(nosevalue)
		...
		cv2.putText(frame, "EAR: {:.3f}".format(ear), (300, 30),
			cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
		cv2.putText(frame, "NAR: {:.3f}".format(nar), (300, 15),
			cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
